...
export default function MyOrdersPage() {
  ...
  return <main className="min-h-screen bg-[var(--white-100)] p-4 md:p-8"><div className="mx-auto max-w-7xl"><div className="mb-8 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-[var(--primary-400)]">{t("myOrders.badge")}</p><h1 className="mt-1 text-3xl font-bold">{t("myOrders.title")}</h1><p className="mt-2 text-sm text-[var(--black-100)]">{t("myOrders.description")}</p></div><div className="flex gap-2"><Link href="/account/profile"><Button variant="outline">{t("myOrders.profile")}</Button></Link><Link href="/my-orders/new"><Button>{t("myOrders.addOrder")}</Button></Link></div></div>
    {loading ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }, (_, i) => <Skeleton key={i} className="h-64 w-full" />)}</div> : orders.length === 0 ? <Card><CardContent className="py-14 text-center"><p className="text-lg font-semibold">{t("myOrders.empty")}</p><Link href="/my-orders/new" className="mt-5 inline-block"><Button>{t("myOrders.addFirstOrder")}</Button></Link></CardContent></Card> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{orders.map((order) => <Card key={order._id} className="h-full"><CardHeader><CardTitle className="line-clamp-2 text-lg">{order.description}</CardTitle></CardHeader><CardContent className="flex h-full flex-col gap-3 text-sm"><div className="rounded-full bg-[var(--primary-100)] px-3 py-1 text-center font-semibold text-[var(--primary-500)]">{t(`orderStatus.${statusKey[order.status]}`)}</div><div className="space-y-2 text-[var(--black-200)]"><p>{t("myOrders.orderDate")}: {dayjs(order.createdAt).format("YYYY-MM-DD")}</p><p>{t("myOrders.materialsCount")}: {order.rawMaterials.length}</p><p>{t("myOrders.cost")}: <strong>{order.totalPrice.toLocaleString()}</strong></p><p>{t("myOrders.deliveryDate")}: {order.expectedFinishDate ? dayjs(order.expectedFinishDate).format("YYYY-MM-DD") : t("myOrders.waitingAdmin")}</p></div><Link href={`/my-orders/${order._id}`} className="mt-auto pt-3"><Button className="w-full" variant="outline">{t("myOrders.details")}</Button></Link></CardContent></Card>)}</div>}
  </div></main>;
}
