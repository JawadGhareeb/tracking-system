...
export default function MyOrderDetailsPage() {
  ...
  return <main className="min-h-screen bg-[var(--white-100)] p-4 md:p-8"><div className="mx-auto max-w-5xl space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-[var(--primary-400)]">{t("myOrderDetails.badge")}</p><h1 className="mt-1 text-3xl font-bold">{t("myOrderDetails.title")}</h1></div><div className="flex gap-2"><Link href="/my-orders"><Button variant="outline">{t("myOrderDetails.myOrders")}</Button></Link>{order.status === "PENDING" ? <Link href={`/my-orders/${order._id}/edit`}><Button variant="outline">{t("myOrderDetails.editOrder")}</Button></Link> : null}<Link href="/my-orders/new"><Button>{t("myOrderDetails.addOrder")}</Button></Link></div></div>
