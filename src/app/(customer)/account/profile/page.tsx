...
export default function CustomerProfilePage() {
  ...
  const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState(""); const [email, setEmail] = useState(""); const [phoneNumber, setPhoneNumber] = useState(""); const [username, setUsername] = useState(""); const [password, setPassword] = useState("");
  const fill = (next: IUser) => { setUser(next); setFirstName(next.fullName.firstName); setLastName(next.fullName.lastName); setEmail(next.email); setPhoneNumber(next.phoneNumber || ""); setUsername(next.username); setPassword(""); };
  ...
  return <main className="min-h-screen bg-[var(--white-100)] p-4 md:p-8"><div className="mx-auto max-w-3xl space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-[var(--primary-400)]">{t("customerProfile.badge")}</p><h1 className="mt-1 text-3xl font-bold">{t("customerProfile.title")}</h1></div><div className="flex gap-2"><Link href="/my-orders"><Button variant="outline">{t("customerProfile.myOrders")}</Button></Link><Link href="/my-orders/new"><Button>{t("customerProfile.addOrder")}</Button></Link></div></div><Card><CardHeader><CardTitle>{t("customerProfile.personalInfo")}</CardTitle></CardHeader><CardContent>{!editing ? <div className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><Info label={t("customerProfile.firstName")} value={user.fullName.firstName} /><Info label={t("customerProfile.lastName")} value={user.fullName.lastName} /><Info label={t("customerProfile.email")} value={user.email} /><Info label={t("customerProfile.phoneNumber", { defaultValue: "رقم الموبايل" })} value={user.phoneNumber || "-"} /><Info label={t("customerProfile.username")} value={user.username} /></div><Button onClick={() => setEditing(true)}>{t("customerProfile.edit")}</Button></div> : <form onSubmit={submit} className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><Field label={t("customerProfile.firstName")} value={firstName} setValue={setFirstName} /><Field label={t("customerProfile.lastName")} value={lastName} setValue={setLastName} /><Field label={t("customerProfile.email")} value={email} setValue={setEmail} type="email" /><Field label={t("customerProfile.phoneNumber", { defaultValue: "رقم الموبايل" })} value={phoneNumber} setValue={setPhoneNumber} type="tel" /><Field label={t("customerProfile.username")} value={username} setValue={setUsername} /><Field label={t("customerProfile.newPasswordOptional")} value={password} setValue={setPassword} type="password" /></div><div className="flex gap-2"><Button type="submit" disabled={saving}>{saving ? t("common.saving") : t("customerProfile.save")}</Button><Button type="button" variant="outline" onClick={() => { fill(user); setEditing(false); }}>{t("customerProfile.cancel")}</Button></div></form>}</CardContent></Card></div></main>;
}
