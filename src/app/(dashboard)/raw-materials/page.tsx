...
const emptyForm: IRawMaterialPayload = { name: "", category: "FABRIC", color: "", unit: "PIECE", stockQuantity: 0, unitPrice: 0, minimumStock: 0 };
...
export default function RawMaterialsPage() {
  ...
  const [items, setItems] = useState<IRawMaterial[]>([]); const [loading, setLoading] = useState(true); const [mutating, setMutating] = useState(false); const [form, setForm] = useState<IRawMaterialPayload>(emptyForm); const [editing, setEditing] = useState<IRawMaterial | null>(null); const [deleting, setDeleting] = useState<IRawMaterial | null>(null); const [dialogOpen, setDialogOpen] = useState(false); const [availabilityFilter, setAvailabilityFilter] = useState<"all" | "available" | "unavailable">("all");
  const load = useCallback(async () => { setLoading(true); try { const response = await rawMaterialsApiService.getAll(availabilityFilter === "all" ? undefined : availabilityFilter); setItems(response.items ?? []); } catch (err) { errorToast({ title: t("rawMaterialsPage.loadFailed"), description: err instanceof Error ? err.message : undefined }); } finally { setLoading(false); } }, [availabilityFilter, errorToast, t]);
  useEffect(() => { void load(); }, [load]);
  const openCreate = () => { setEditing(null); setForm(emptyForm); setDialogOpen(true); };
  const openEdit = (item: IRawMaterial) => { setEditing(item); setForm({ name: item.name, category: item.category, color: item.color, unit: item.unit, stockQuantity: item.stockQuantity, unitPrice: item.unitPrice, minimumStock: item.minimumStock }); setDialogOpen(true); };
