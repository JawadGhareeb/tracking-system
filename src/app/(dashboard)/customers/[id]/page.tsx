...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      try {
        const response = await usersApiService.getById(customerId);
        const user = normalizeUser(response, customerId);
        if (cancelled) {
          return;
        }
  ...
  const onSubmit = async (values: EditEmployeeFormValues) => {
    ...
    try {
      await updateUser(customerId, payload);
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-[var(--black-300)]">{t("dashboardUsers.editCustomer")}</h1>
      </div>
      ...
      <Card className="max-w-5xl border-none bg-transparent shadow-none">
        <CardHeader>
          <CardTitle className="text-2xl">{t("dashboardUsers.customerInformation")}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoadingUser ? (
